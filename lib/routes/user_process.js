...
function update(req, res, next) {
  ...
  return user.update()
      .then(() => {
        return (payload.passwdold && payload.passwd) ? user.findByEmail(payload.email) : false;
      })
      .then((userFound) => {
        return (userFound) ? userFound.verifyPassword(payload.passwdold) : true;
      })
      .then((doNotUpdatePassword) => {
        return (doNotUpdatePassword) ? true : user.updatePassword();
      })
