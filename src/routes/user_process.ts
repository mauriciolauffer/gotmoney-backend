...
export async function update(c: Context) {
  ...
  try {
    await user.update();
    const userFound = (payload.passwdold && payload.passwd) ? await user.findByEmail(payload.email) : false;
    const isPasswordVerified = (userFound) ? await userFound.verifyPassword(payload.passwdold).then(() => true).catch(() => false) : true;
